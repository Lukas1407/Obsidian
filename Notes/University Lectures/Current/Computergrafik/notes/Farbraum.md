...
- Jede der drei Primärfarben wird durch eine spezifische Wellenlänge dargestellt (z.B. 645 nm für Rot, 525 nm für Grün und 444 nm für Blau).
...
- Dieser Farbraum arbeitet mit **subtraktiver Farbmischung** und wird für Drucker und Maltechniken verwendet, da Pigmente und Tinten Licht absorbieren statt es zu emittieren.
- Jede Primärfarbe des CMY-Modells absorbiert ein Drittel des Spektrums:
  - **Cyan** absorbiert Rot.
  - **Magenta** absorbiert Grün.
  - **Gelb** absorbiert Blau.
- In der Praxis wird oft eine vierte Farbe, **Schwarz (K)**, hinzugefügt, um tiefere Schwarztöne zu erzeugen und die Menge an Tinte zu reduzieren.
- Die Formel für Schwarz im CMYK-Modell ist $K = \min(C, M, Y)$, und die anderen Farben werden entsprechend angepasst.
...
- Das HSV-Modell wird oft als Zylinder oder Kegel dargestellt und ist besonders nützlich für **Benutzeroberflächen**, da es die Farbwahl intuitiver macht.
- Es gibt eine einfache Konvertierung zwischen RGB und HSV, wobei $V = \max(r, g, b)$, $S = (\text{max} - \text{min}) / \text{max}$, und der Farbton $H$ basierend auf dem größten Wert der RGB-Komponenten berechnet wird.
![[Pasted image 20241104082531.png|200]]

## XYZ-Farbraum
Der **XYZ-Farbraum** wurde von der **CIE (Commission Internationale de l'Eclairage)** entwickelt, um einen standardisierten Farbraum zur Verfügung zu stellen, der alle wahrnehmbaren Farben beschreibt und eine einfache Konvertierung zwischen verschiedenen Farbräumen ermöglicht. Hier sind die Hauptpunkte:
1. **Ziel des XYZ-Farbraums**:
   ...
   - Er basiert auf den sogenannten **Color Matching Functions** und repräsentiert Farben unabhängig von spezifischen Geräten oder Systemen.
2. **Eigenschaften des XYZ-Farbraums**:
   - Die Color Matching Functions für den XYZ-Farbraum, dargestellt als $\overline{x}(\lambda)$, $\overline{y}(\lambda)$ und $\overline{z}(\lambda)$, sind positiv und wurden so festgelegt, dass sie eine lineare Kombination der ursprünglichen RGB Color Matching Functions bilden, jedoch ohne negative Werte. Das erleichtert Berechnungen.
   - Die Y-Komponente wurde so gewählt, dass sie die Helligkeit (Luminanz) repräsentiert und somit der Luminanzkurve $\overline{y}(\lambda)$ entspricht.
   - Die $\overline{z}(\lambda)$-Komponente entspricht in etwa der Empfindlichkeit des S-Zapfens für Blau, während $\overline{x}(\lambda)$ die Rot- und Grünbereiche abdeckt.
...
4. **Übertragbarkeit auf RGB**:
   - Der XYZ-Farbraum kann durch eine **lineare Transformation in den RGB-Farbraum** umgewandelt werden, was eine einfache Konvertierung ermöglicht. Diese Transformation wird durch eine Matrix dargestellt, die das XYZ-System in RGB-Werte übersetzt. Die Umkehrung dieser Matrix (für die Umrechnung zurück) kann jedoch negative Werte enthalten, was bedeutet, dass manche XYZ-Werte auf nicht darstellbare RGB-Werte abgebildet werden (da sie physisch nicht möglich sind).
5. **Imaginary Primaries**:
   - Die Festlegung der XYZ Color Matching Functions führt zu **"imaginären Primärfarben"** (übersättigte Farben), die physisch nicht realisierbar sind. Das bedeutet, dass der XYZ-Farbraum Farben beschreibt, die jenseits des für das Auge sichtbaren Spektrums liegen, um den gesamten wahrnehmbaren Farbbereich mathematisch abzudecken.
### Chromatizität: Vom CIE XYZ zum CIE xyY Farbraum
Der CIE xyY-Farbraum ist eine Transformation des XYZ-Farbraums, die es ermöglicht, Farben unabhängig von ihrer Helligkeit zu beschreiben. Das Ziel dieser Transformation ist es, nur den Farbton und die Sättigung darzustellen, ohne die Helligkeit zu berücksichtigen.
#### Beobachtungen im XYZ-Farbraum
- Im XYZ-Farbraum repräsentieren alle Werte $kX, kY, kZ$ (wobei $k > 0$ ein Skalierungsfaktor ist) dieselbe Farbe mit unterschiedlicher Intensität. Das bedeutet, dass diese Werte lediglich in ihrer Helligkeit variieren, aber denselben Farbton und dieselbe Sättigung haben.
#### Normalisierung und Projektion
1. **Normalisierung**: Um nur die Farbanteile darzustellen, wird eine Normalisierung durchgeführt, bei der $X + Y + Z = 1$ gesetzt wird. Dadurch erhält man einen Farbpunkt, der von der Helligkeit unabhängig ist und nur Farbton und Sättigung repräsentiert.
2. **Projektion auf die XY-Ebene**: Nach der Normalisierung wird der Punkt auf die XY-Ebene projiziert (Z wird weggelassen). Somit erhalten wir die sogenannten **x**- und **y**-Werte, die die Chromatizität (Farbton und Sättigung) beschreiben.
...
Das resultierende **xy-Diagramm** oder **Chromatizitätsdiagramm** zeigt alle wahrnehmbaren Farben als eine zweidimensionale Fläche:
- Die **äußere Linie** des Diagramms zeigt die Spektralfarben, also die Farben mit maximaler Sättigung bei verschiedenen Wellenlängen.
...
- Der **Weißpunkt** befindet sich etwa in der Mitte des Diagramms und stellt eine Mischung aller Wellenlängen in ausgewogenen Anteilen dar. Er entspricht einem neutralen Weißlicht (ähnlich dem Sonnenlicht).
...
- Alle Farben innerhalb einer Linie oder eines Dreiecks können durch additive Farbmischung der Endpunkte dieser Linie oder Dreiecksfläche erzeugt werden.
![[Pasted image 20241104084526.png|200]]
- Durch eine Linie, die vom Weißpunkt durch einen Punkt verläuft, erhält man die reine Farbe dieses Punktes.
...
- Die **Spektralfarben** befinden sich entlang der Randkurve des Chromatizitätsdiagramms und repräsentieren Farben mit maximaler Sättigung für jede Wellenlänge des sichtbaren Spektrums.
    - Diese Farben sind „rein“ und bestehen nur aus einer einzigen Wellenlänge. Sie werden auch als **monochromatische Farben** bezeichnet.
...
Falls die Helligkeit (Y) gegeben ist und die Farbe durch die Werte $x$ und $y$ spezifiziert ist, können die ursprünglichen XYZ-Werte wie folgt berechnet werden:
$$
...
## Gamut-Mapping
- Der darstellbare Farbbereich eines Geräts (Gamut) ist durch ein Dreieck im Diagramm definiert, das von den Primärfarben des Geräts aufgespannt wird.
...
- Da verschiedene Geräte unterschiedliche Gamuts haben, wird das **Gamut-Mapping** verwendet, um Farben konsistent zwischen verschiedenen Geräten darzustellen. Dadurch wird vermieden, dass Farben außerhalb des Gamuts auf einem Gerät anders dargestellt werden.
## L\*a\*b\* Farbraum
Das **CIE L\*a\*b\* Modell** ist ein psychophysisches Farbmodell, das entwickelt wurde, um Farben in einer Weise darzustellen, die der menschlichen Farbwahrnehmung näher kommt. Es wurde speziell entwickelt, um Farbunterschiede wahrnehmungsgerecht abzubilden. Hier sind die Hauptmerkmale:
